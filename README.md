...
Minimap2 v2.30 (Li, 2018) was used with options “-ax map-ont” to align raw reads and consensus genome to the S. enterica reference genome (RefSeq GCF_000006945.2) and was outputted to Sequence Alignment/Map (SAM) file. Subsequently, SAMtools v1.22.1 (Li et al., 2009) was applied to convert the resulting SAM files to sorted and indexed Binary Alignment/Map (BAM) files. Clair3 (Zheng et al., 2022) was  used for variant calling. The genome browser, IGV (Robinson et al., 2011, 2017), will be deployed to visualize variants with inputs from the reference genome, aligned BAM files, and variant call format (vcf) files. Proksee's web interface (Grant et al., 2023) was employed to visualize the entire assembled genome. Bcftools v1.22 (Li, 2011) plot-vcfstats was used to visualize SNPS and indels.
